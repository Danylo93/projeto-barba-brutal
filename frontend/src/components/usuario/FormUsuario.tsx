...
export default function FormUsuario() {
    ...
    const [tenantId, setTenantId] = useState<number>(
        parseInt(process.env.NEXT_PUBLIC_TENANT_DEFAULT_ID || '1', 10)
    )
    ...
    useEffect(() => {
        if (usuario?.email && modo === 'entrar') {
            // Se já está logado e está tentando fazer login, redireciona para home
            const dest = params.get('destino') as string
            router.push(dest ? dest : '/')
        }
    }, [usuario, router, params, modo])

    async function submeter() {
        if (modo === 'entrar') {
            await entrar({ email, senha, tenantId })
        } else {
                    ...
                    <div className="flex flex-col gap-4 rounded">
                        ...
                        {modo === 'cadastrar' && (
                            <input
                                ...
                                onChange={(s) =>
                                    setTelefone(formatarTelefoneInput(s.target.value))
                                }
